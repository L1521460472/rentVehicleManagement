      ...
      <div class="footerNav">
        ...
        <div class="formNav">
          <el-form ref="form" :model="form" label-width="130px" label-position="right">
            <el-form-item label="审核结果" style="width:500px">
               <span v-if="form.status==0">待审核</span>
               <span v-if="form.status==1">审核通过</span>
               <span v-if="form.status==2">审核不通过</span>
            </el-form-item>
            <el-form-item label="审核意见"  >
              <el-input disabled maxlength="500" style="width:800px"
                type="textarea" :rows="5" size="small" v-model="form.auditSuggestion" ></el-input>
            </el-form-item>
          </el-form>
        </div>
      </div>
  ...
  methods: {
    ...
    getInitData() {
      ...
      axios({
        method: "get",
        url:
          "/vehicle-service/backOffVehicleInfo/returnCarSettlementDetail?id=" +
          this.$route.query.id,
        headers: this.headers,
      })
        .then((result) => {
          ...
          if (result.data.status === 0) {
            ...
            // remakeList:[],
            if(result.data.data.remakeList&&result.data.data.remakeList.length==10){
            ...
            this.form.statusStr=result.data.data.auditStatusStr
            this.form.status=result.data.data.auditStatus
