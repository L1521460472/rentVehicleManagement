  ...
  <div class="container">
     ...
     <el-table :data="dataList" border class="table" stripe :height="tableHeight" size="small"
      ...
      <el-table-column type="index" label="序号" width="50" align="center">
       ...
       <el-table-column prop="relationVehicleNums" label="关联车辆数量"></el-table-column>
       <el-table-column prop="assignOrderNums" label="分配订单数量"></el-table-column>
       <el-table-column prop="relationContractNums" label="关联合同数量"></el-table-column>
       <el-table-column prop="relationContractOverdueNums" label="关联合同现存逾期数量"></el-table-column>
       <el-table-column prop="relationContractViolationNums" label="关联合同现存违章数量"></el-table-column>
     </el-table>
...
import getEnterpriseBox from '@/components/aacommon/getEnterpriseBox.vue'
  export default{
    ...
    methods:{
      onexport(){
        axios({ method: "get",
         url: `/vehicle-service/vehicleInfo/salesManRelationStatisticsExport?enterpriseId=${this.search.enterpriseId}`,
         headers: this.headers, data:this.search, responseType: 'blob'})
          .then((result) => {
            if (result.data.type === "application/json") {
                ...
                link.download = `存车地点车辆数报表${formatDate(new Date(),"YYYYMMDDHHmmss")}.xls`;
      ...
      getlist(){
          ...
          axios({
            method: "post",
            url:'/vehicle-service/vehicleInfo/salesManRelationStatistics',
            data:this.search,
