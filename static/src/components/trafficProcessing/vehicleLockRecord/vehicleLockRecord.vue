  ...
  <div id="header" v-loading="loading">
    <div class="header">
      <div class="headerTop scoped">
        ...
        <div class="nav">
          ...
          <makeSelect :options="lockStatusOption" v-model="searchdata.operateType"></makeSelect>
        </div>
        <div class="nav">
    ...
    <div class="footer">
      <el-table :data="dataList"  border stripe :header-cell-style="{ background: '#F5F7FA', color: '#333333' }"
        ...
        <el-table-column prop="operateTypeStr" label="操作类型"></el-table-column>
        <el-table-column prop="createdName" label="操作账号"></el-table-column>
        <el-table-column prop="createTime" label="操作时间"></el-table-column>
        <el-table-column prop="" label="记录" align="center">
           <template slot-scope="scope">
             <a style="text-decoration: none;color:#409EFF" href="javascript:;" @click="detail(scope.row)" v-if="detailBtn">查看</a>
           </template>
        </el-table-column>
        <el-table-column prop="resultDataStr" label="操作结果" align="center"></el-table-column>
      </el-table>
    ...
    <el-dialog title="记录" :close-on-click-modal="true" :close-on-press-escape="true"
      :visible.sync="dialogVisible" width="40%" :destroy-on-close="true"
      :before-close="handleClose">
      <vehicleLockRecordDetail :id="id"></vehicleLockRecordDetail>
    </el-dialog>
  </div>
</template>
<script>
...
import { getCookie, dateToString, getMenuBtnList,formatDate } from "@/public";
  import vehicleLockRecordDetail from '@/components/trafficProcessing/vehicleLockRecord/vehicleLockRecordDetail.vue'
export default {
    name: "vehicleLockRecord",components:{vehicleLockRecordDetail},
  data() {
    return {
        dialogVisible:false,
        datestr:null,
  ...
  methods: {
    ...
    getlist(){
      ...
      axios({
        method: "post",
        url: "/vehicle-service/vehicleLockRecord/pageList",
    ...
    menuDataList:{
      handler(data){
          ...
          btns.map(item=>{
            ...
            if(item.name == '查看') this.detailBtn = true
          })
      },
      immediate:true,
      deep:true
