      ...
      <div class="container-l">
         <div class="form2">
            ...
            <div class="item2" style="justify-content: center;">
              <el-button type="primary" v-if="btn1" @click="suoche" :disabled="disabled">锁车</el-button>
            </div>
         </div>
      </div>
    </div>
...
<script>
  ...
  export default{
    ...
    methods:{
      ...
      suoche(){
        if(!this.textarea){
          this.$message.info("请填写锁车原因")
          this.$refs.textarea1.focus()
          return
        }
           this.$confirm('确定发送锁车指令吗？', '提示', {
              ...
              .then((respone)=>{
                if(respone.data&&respone.data.status==0){
                  this.$message.success("已发送锁车指令")
                  this.btn1=false
                  this.$emit("callback")
                }
                else{
                  this.$message.error("发送锁车指令失败，请稍后再试")
                  this.disabled=false
                }
    ...
    watch:{
      menuDataList:{
        handler(data){
            ...
            btns.map(item=>{
              if(item.name == '锁车') this.btn1 = true
            })
        },
        immediate:true,
        deep:true
