  ...
  <div id="operationManagement" v-loading="loading"  element-loading-text="loading">
      <div class="header" v-if="international.global">
        <div class="headerTop scoped">
            ...
            <div class="nav">
                <span class="demonstration">{{international.content.content_operationManagement_managementName}}</span>
                <el-input maxlength="50" size="small" v-model="searchName" :placeholder="international.content.content_operationManagement_managementName"></el-input>
            </div>
            <div class="nav">
                <span class="demonstration">{{international.content.content_operationManagement_status}}</span>
                <el-select clearable size="small" v-model="status" value-key="value"  :placeholder="international.content.content_operationManagement_status" >
                    <el-option
      ...
      <div class="footer" v-if="international.field">
          <div class="top">
            ...
            <el-button size="small" :class="{ 'active': !isDisable }" v-if="deactiveBtn" :disabled="isDisable" @click="handleLoseEfficacy" >
          ...
          <div class="bottom">
            <div class="footerTable">
                ...
                >
                    ...
                    <el-table-column
                    prop="id"
                    :label="international.field.field_operationManagementList_serialNumber"
                    align="center"
                    width="60">
                    ...
                    <el-table-column
                    prop="enterpriseCode"
                    :label="international.field.field_operationManagementList_managementCode"
                    width="140"
                    show-overflow-tooltip>
                    </el-table-column>
                    <el-table-column
                    prop="enterpriseName"
                    :label="international.field.field_operationManagementList_managementName"
                    width="180"
                    show-overflow-tooltip>
                    </el-table-column>
                    <el-table-column
                    prop="statusText"
                    :label="international.field.field_operationManagementList_status"
                    width="100"
                    show-overflow-tooltip>
                        <template slot-scope="scope">
                    ...
                    :label="international.field.field_operationManagementList_belongOrganization"
                    width="200"
                    show-overflow-tooltip>
                    </el-table-column>
                    <el-table-column
                    prop="headOfPerson"
                    :label="international.field.field_operationManagementList_principal"
                    width="130"
                    show-overflow-tooltip>
                    </el-table-column>
                    <el-table-column
                    prop="mobile"
                    :label="international.field.field_operationManagementList_phone"
                    width="130"
                    show-overflow-tooltip>
                    </el-table-column>
                    <el-table-column
                    prop="email"
                    :label="international.field.field_operationManagementList_email"
                    min-width="160"
                    show-overflow-tooltip>
                    </el-table-column>
                </el-table>
...
import {getOperationManagementData,activeManagement,loseEefficacyManagement,getStatusList} from '../../api/fileManagement/api'
export default {
    name:'operationManagement',
    data() {
        return {
            isDisable:true,//是否禁用编辑激活失效
            total:0, //数据总条数
            currentPage:1,//当前页数
            pageSize:10, //每页长度
            searchCode:'',//管理分支机构编码
    ...
    methods: {
        ...
        handleActive(row){
            ...
            this.$confirm(this.international.global.global_confirmActive,{
                ...
                activeManagement(params,this.headers).then(res=>{
                    if(res.status == 0){
        ...
        // 失效状态
        handleLoseEfficacy(row){
            let params = this.ids
            this.$confirm(this.international.global.global_confirmDeactive,{
                confirmButtonText: this.international.global.global_confirm,
                cancelButtonText: this.international.global.global_cancel,
            }).then(()=>{
                loseEefficacyManagement(params,this.headers).then(res=>{
                    if(res.status == 0){
                        this.$message.success({
                            message:this.international.global.global_changeStatusSuccess,
                            center:true
        ...
        //                 enterpriseCode: this.addform.enterpriseCode,
        //                 parentId: this.addform.belong.id,
        //                 enterpriseStatus: this.addform.status.id,
        //                 headOfPerson: this.addform.name.id,
        ...
        // // 编辑时改变状态
        // changeStatus(id){
        //     this.editform.enterpriseStatus = id
        // },
        // // 编辑时改变负责人
        // changePrincipal(id){
        //     this.editform.headOfPerson = id
        // },
        // 编辑确认提交
        // editConfirm(){
        ...
        //                 enterpriseCode:this.editform.enterpriseCode,
        //                 parentId: this.editform.parentId,
        //                 enterpriseStatus: this.editform.enterpriseStatus,
        //                 headOfPerson: this.editform.headOfPerson,
    ...
    watch:{
        ...
        "$store.getters.isStatus": function () {
            if (this.$store.state.isStatus == true && this.$route.path == '/addOperation') {
                this.getListData();
            }
        },
    },
...
#operationManagement >>> .el-dialog__footer{
    text-align: center ;
}
</style>