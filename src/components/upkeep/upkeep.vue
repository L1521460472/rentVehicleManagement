  ...
  <div id="header" v-loading="loading">
    <div class="header">
      <div class="headerTop scoped">
        <div class="nav">
          ...
          <!-- <getEnterpriseBox v-model="enterpriseId" type="2"></getEnterpriseBox> -->
        </div>
        <div class="nav">
          <span class="demonstration">保养时间</span>
           <el-date-picker style="width: 250px;" v-model="upkeepTime" type="daterange" value-format="yyyy-MM-dd"
           start-placeholder="开始日期" end-placeholder="结束日期"></el-date-picker>
        </div>
        <div class="nav">
          <span class="demonstration">添加人</span>
          <getTianjiaren v-model="createBy"></getTianjiaren>
        </div>
        <div class="nav">
        ...
        <div class="nav">
          ...
          <el-input v-model="vehicleNo"></el-input>
        </div>
        <div class="nav">
          <span class="demonstration">下次保养时间</span>
           <el-date-picker style="width: 250px;" v-model="nextUpkeepTime" type="daterange" value-format="yyyy-MM-dd"
           start-placeholder="开始日期" end-placeholder="结束日期"></el-date-picker>
        </div>
        <div class="nav">
          <span class="demonstration">核实状态</span>
          <makeSelect v-model="auditStatus" :options="[{name:'待核实',value:0},{name:'核实通过',value:1},{name:'核实不通过',value:2}]"></makeSelect>
        </div>
    ...
    <div class="footer">
      ...
      <el-table :data="dataList"  border stripe :header-cell-style="{ background: '#F5F7FA', color: '#333333' }"
        ...
        <el-table-column prop="maintainTime" label="保养时间" width="140"></el-table-column>
        ...
        <el-table-column prop="payFee" label="实际支付（元）" width="110"></el-table-column>
      ...
      <div class="footer_page">
        <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="currentPage"
          :page-sizes="[10, 20, 50, 100, 200]" :page-size="pageSize" :pager-count="5"
...
export default {
  ...
  methods: {
    shenhe(){
      ...
      this.$router.push({path:'/auditUpkeep',query:{id:row.id}})
    },
    detail(){
      let row=this.multipleSelection[0]
      this.$router.push({path:'/lookUpkeep',query:{id:row.id}})
    },
    add(){
      this.$store.commit("changeIsStatus", false);
      this.$router.push({path:'/addUpkeep'})
    },
    ...
    getlist(){
      let params ={
        ...
        maintainStartDate: this.upkeepTime.length < 1 ? null:this.upkeepTime[0],
        maintainEndDate: this.upkeepTime.length < 1 ? null:this.upkeepTime[1],
        nextMaintainStartDate: this.nextUpkeepTime.length < 1 ? null:this.nextUpkeepTime[0],
        nextMaintainEndDate: this.nextUpkeepTime.length < 1 ? null:this.nextUpkeepTime[1],
  ...
  watch: {
    ...
    "$store.getters.isStatus": function () {
      if (
        ...
        (this.$route.path == "/addUpkeep" || this.$route.path == "/auditUpkeep")
      ) {
        this.getlist();
      }
    },
