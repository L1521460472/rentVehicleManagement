        ...
        <div class="item">
          ...
          <div class="itemcontant">
              ...
              <div class="itemitem">
                ...
                <div class="itemitem-r">
                  <el-input v-model="adddata.errorCode"></el-input>
                </div>
              </div>
              <br>
              <div class="itemitem">
                ...
                <div class="itemitem-r" >
                  <el-input v-model="adddata.repairMileage"></el-input>
                </div> km
              </div>
              <br>
              <div class="itemitem">
          ...
          <div class="itemcontant">
              <div class="itemitem">
                ...
                <div class="itemitem-r">
                  ...
                  placeholder="自动计算总花费" style="width:150px;margin-right:5px;"></el-input>元
                </div>
              </div>
              <br>
              <div class="itemitem">
                ...
                <div class="itemitem-r">
                  ...
                  placeholder="请输入实际支付费用" style="width:150px;margin-right:5px"></el-input>元
                </div>
              </div>
              <br>
              <div class="itemitem">
                ...
                <div class="itemitem-r">
                  <el-input v-model="adddata.remark"  type="textarea"
                  placeholder="请输入备注（100字以内）" style="width:500px" :autosize="true"></el-input>
                </div>
              </div>
              <br>
              ...
              <div class="itemitem" style="margin-bottom: 25px;display: block;">
                <div style="width: 100%;text-align: center;">
                  <el-button type="primary" @click="submmit()" v-if='btn1' :disabled="cantijiao">确认</el-button>
...
<script>
  ...
  export default{
    ...
    data(){
      return {
          ...
          qitaweixiuitems:[
            ...
            {checked:false,type:5,name:'更换后视镜',count:null},
            {checked:false,type:6,name:'修换发动机',count:null},
            {checked:false,type:7,name:'更换电瓶',count:null}
          ],
          zidingyiitems:[],
          btn1:false,
          addurl:"/vehicle-service/repairManagement/insertRepair",
          headers: {
          ...
          adddata:{
            ...
            'contractId':null,
            "ids": [],
            'insertCustomRepairDetailDtoList':[],
            'insertOtherRepairDetailDtoList':[],
            'insertRepairDetailDtoList':[],
            "payFee": null,
            "remark": "",
            "repairMileage": null,
            "repairTime": "",
    ...
    showorhidden(flag,e){
      ...
      submmit(){
        ...
        if(this.adddata.vehicleId){
          if(!this.adddata.repairTime){
            this.$message.error('请填写维修时间！')
            this.cantijiao=false
            return
          }
          if(!this.adddata.repairMileage){
            this.$message.error('请填写维修时里程！')
            this.cantijiao=false
            return
          }
          if(!(new RegExp("^[1-9][0-9]*$").test(this.adddata.repairMileage))){
            this.$message.error('保养时里程为正整数！')
            this.cantijiao=false
            return
          }
          this.adddata.insertRepairDetailDtoList=[]
          this.adddata.insertOtherRepairDetailDtoList=[]
          this.adddata.insertCustomRepairDetailDtoList=[]

          for(let item of this.changguiitems){
            if(item.checked&&item.count){
               this.adddata.insertRepairDetailDtoList.push({"commonRepair": item.type,"commonRepairCost": item.count})
            }
          }
          for(let item of this.qitaweixiuitems){
            if(item.checked&&item.count){
              this.adddata.insertOtherRepairDetailDtoList.push({"otherRepair": item.type,"otherRepairCost": item.count})
            }
          }
          for(let item of this.zidingyiitems){
            console.log(this.zidingyiitems)
            this.adddata.insertCustomRepairDetailDtoList.push({"customRepair": item.name,"customRepairCost": item.count})
          }
    ...
    watch:{
      ...
      menuDataList:{
        handler(data){
            let btns = getMenuBtnList(data,"/maintenance")
            btns.map(item=>{
