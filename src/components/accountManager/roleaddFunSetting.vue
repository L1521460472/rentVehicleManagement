    ...
    <div class="footer">
      ...
      <div class="footerNav" v-if="international.title">
        <el-form ref="form" :model="form" label-width="125px" class="from" label-position="right">
            <el-form-item class="formItem" prop="enterpriseId"
            :rules="[{ required: true,message:'所属公司不能为空', trigger: 'blur'}]" label="所属公司" >
               <el-select size="small" filterable v-model="form.enterpriseId">
                 <el-option v-for="item in optionsAffiliation" :key="item.id" :label="item.name" :value="item.id"  >
                 </el-option>
               </el-select>
            </el-form-item>
            <el-form-item class="formItem" prop="roleName"
            ...
            <el-form-item style="display: none;" class="formItem" prop="roleType"
            ...
            :label="international.content.content_functionPermissionSettings_roleType">
                <el-select size="small" v-model="form.roleType"  placeholder="" >
                    <el-option v-for="item in roleTypeList" :key="item.id" :label="item.value" :value="item.id"></el-option>
                </el-select>
            </el-form-item>
...
import {getFunDeail,addFunSetting2,editFunSetting,getTypeList} from '../../api/userPermissions/api'
export default {
